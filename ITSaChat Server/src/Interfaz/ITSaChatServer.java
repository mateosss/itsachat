...
public class ITSaChatServer extends javax.swing.JFrame {

    Server server;

    public ITSaChatServer() throws IOException, ClassNotFoundException {
        initComponents();
        ...
        if (datos
                != null) {
            ServerProperties.setIpFtp((String) datos[0]);
            ServerProperties.setPuertoFtp((String) datos[1]);
            ServerProperties.setBlacklist((String) datos[2]);
            ServerProperties.setBlacklistOn((boolean) datos[3]);
            ServerProperties.setWhitelist((String) datos[4]);
            ServerProperties.setWhitelistOn((boolean) datos[5]);
            ServerProperties.setNombre((String) datos[6]);
            ServerProperties.setEstado((String) datos[7]);
            ServerProperties.setPuerto((String) datos[8]);
            ServerProperties.setUserFtp((String) datos[9]);
            ServerProperties.setPswFtp((String) datos[10]);
            ServerProperties.setRutaDes((String) datos[11]);
            ServerProperties.setRutaFtp((String) datos[12]);
            ServerProperties.setInicio((boolean) datos[13]);
        }
    }
    ...
    private void initComponents() {
        ...
        iniciar.setToolTipText("Iniciar Servidor");
        ...
        output.setFont(new java.awt.Font("Ubuntu", 0, 8)); // NOI18N
    ...
    private void enviarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_enviarActionPerformed
        if (server != null) {
            ...
            server.enviarMensaje(null, ServerProperties.getNombre() + " > " + texto);
    ...
    private void archivoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_archivoActionPerformed
        if (server != null) {
            ...
            if (respuesta == JFileChooser.APPROVE_OPTION) {
                ...
                server.enviarMensaje(null, ServerProperties.getNombre() + " > /archivo " + archivoElegido.getPath());
    ...
    private void opcionesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_opcionesActionPerformed
        try {
            ...
            Logger.getLogger(ITSaChatServer.class
                    .getName()).log(Level.SEVERE, null, ex);
        }
    }//GEN-LAST:event_opcionesActionPerformed

    private void iniciarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_iniciarActionPerformed
//        if (this.iniciar.getToolTipText().equals("Iniciar Servidor")) {
        ServerProperties.setExit(false);
        server = new Server("Server", this);
        server.start();
        server.enviarMensaje(null, ServerProperties.getNombre() + " > Servidor Iniciado!");
        this.iniciar.setEnabled(false);
        this.iniciar.setToolTipText("Servidor Iniciado. Cierra la ventana para pararlo.");
        this.iniciar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Resource/servidorIniciado.png")));
//            this.iniciar.setToolTipText("Terminar Servidor");
//            this.iniciar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Resource/obtenerConversaciones.png")));
//        } else {
//            try {
//                System.out.println("1");
//                for (int i = 0; i < ServerProperties.getConexiones().length; i++) {
//                    System.out.println("2");
//                    System.out.println(ServerProperties.getConexiones()[i].getName());
//                    System.out.println("3");
//                    ServerProperties.getConexiones()[i].cerrarConexion();
//                    System.out.println("pataaa");
//                }
//                System.out.println("4");
//                ServerProperties.getServer().close();
//                System.out.println("5");
//                ServerProperties.setExit(true);
//                System.out.println("6");
//            } catch (IOException ex) {
//                Logger.getLogger(ITSaChatServer.class.getName()).log(Level.SEVERE, null, ex);
//            }

//            this.iniciar.setToolTipText("Iniciar Servidor");
//            this.iniciar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Resource/descargas.png")));
//        }

    }//GEN-LAST:event_iniciarActionPerformed

    private void archivosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_archivosActionPerformed
        String[] firefox = {"firefox", "ftp://" + ServerProperties.getUserFtp() + ":" + ServerProperties.getPswFtp() + "@" + ServerProperties.getIpFtp() + ":" + ServerProperties.getPuertoFtp() + "/ITSaChat/Archivos/"};
        String[] chromium = {"chromium-browser", "ftp://" + ServerProperties.getUserFtp() + ":" + ServerProperties.getPswFtp() + "@" + ServerProperties.getIpFtp() + ":" + ServerProperties.getPuertoFtp() + "/ITSaChat/Archivos/"};
        String[] chrome = {"google-chrome", "ftp://" + ServerProperties.getUserFtp() + ":" + ServerProperties.getPswFtp() + "@" + ServerProperties.getIpFtp() + ":" + ServerProperties.getPuertoFtp() + "/ITSaChat/Archivos/"};
    ...
    private void formWindowClosing(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowClosing
        if (salir) {
            ...
            try {
                SaveLoad.addHistorial(this.output.getText());

                salir = false;
                Runnable exec = new Runnable() {
                    @Override
                    public void run() {
                        JOptionPane.showMessageDialog(null, "Guardando Historial en servidor FTP", "Guardando historial, para cancelar cierre otra vez", 1);
                    }
                };
                new Thread(exec, "Saliendo").start();

                FTP.subir(ServerProperties.getIpFtp(), Integer.parseInt(ServerProperties.getPuertoFtp()), ServerProperties.getUserFtp(), ServerProperties.getPswFtp(), "/ITSaChat/Interno/Historial/", "historial.hs");
            } catch (IOException | ClassNotFoundException ex) {
                System.out.println("No se ha podido guardar el Historial");
            }
        }
        System.exit(0);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ITSaChatServer.class
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                try {
                    new ITSaChatServer().setVisible(true);
    ...
    public StyledDocument getOutputDocument() {
        return this.output.getStyledDocument();
    }
